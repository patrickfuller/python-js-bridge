 ...
 * (I've added this extra logic to simplify expanding the python-js bridge)
 */
var client = {
    queue: {},
        ...
        this.socket.on("message", function (data) {
            ...
            if (data.error) {
                ...
                if (current.length === 0) {
                    ...
                    updated = current.substring(current.length - 100) + ", " + data.result;
                } else {
                    updated = current + ", " + data.result;
                }
                $(".answer").html(updated);
                $(".number").val(data.result);
            } else {
    ...
    count: function (data) {
        var uuid = this.uuid();
        this.socket.send({method: "count", id: uuid, params: {number: data}});
